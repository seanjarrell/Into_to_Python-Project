...
# ---- Load AI Models ----
# @st.cache_resource means the models only load once, not on every page refresh
@st.cache_resource
def load_engine():
...
def scan_receipt(image_source):
    ...
    # If nothing was detected, just use the whole image as one receipt
    if not crops:
        crops = [image]
    ...
    for i, crop in enumerate(crops):
        ...
        with info_col:
            ...
            # Show a preview of what the filename will be
            filename = make_filename(store, date)
            st.caption(f"Will save as: {filename}")

            # Step 6: Save button — only downloads when the user clicks it
